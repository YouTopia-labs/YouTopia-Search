...
import { proxySerper, proxyCoingecko, callMistralApiWorker } from '../worker.js'; // Import proxy functions from worker.js
import { wikipediaSearch } from '../tools/wikipedia_tool.js'; // Import wikipediaSearch for wikiEye internal use

// Helper to execute tools within the worker environment
async function executeTool(toolName, query, params = {}, env, callAgentFunc) { // Pass env and callAgentFunc
  console.log(`Executing tool: ${toolName} with query: ${query} and params:`, params);
  
  switch (toolName) {
    case 'serper_web_search':
      const serperWebPayload = {
        type: 'search',
        body: { q: query, ...params }
      };
      const webResponse = await proxySerper(serperWebPayload, env);
      if (!webResponse.ok) {
    ...
    case 'serper_news_search':
      const serperNewsPayload = {
        type: 'news',
        body: { q: query, ...params }
      };
      const newsResponse = await proxySerper(serperNewsPayload, env);
      if (!newsResponse.ok) {
        const errorText = await newsResponse.text();
        throw new Error(`Serper API error: ${newsResponse.status} - ${errorText}`);
      }
    ...
    case 'coingecko':
      const coingeckoPayload = {
        params: { ids: query, vs_currencies: 'usd', ...params }
      };
      const coingeckoResponse = await proxyCoingecko(coingeckoPayload, env);
      if (!coingeckoResponse.ok) {
        const errorText = await coingeckoResponse.text();
        throw new Error(`Coingecko API error: ${coingeckoResponse.status} - ${errorText}`);
      }
    ...
    case 'wiki_eye':
      // wikiEye needs the internal callAgent function
      console.log(`Calling wikiEye for query: ${query}`);
      // Assuming wikiEye takes searchResults (which would be passed from orchestrator) and userQuery
      // and also needs the callAgent function for its internal Agent 2 call.
      // This part might need adjustment based on how wikiEye is truly used.
      // For now, it's called with webSearchResults later in orchestrateAgents
      return { data: [] }; // This case should ideally not be hit directly from search_plan, but from orchestrateAgents loop
...
export async function callAgent(model, prompt, input, env, retryCount = 0, streamCallback = null) { // Pass env
  ...
  try {
    // Use callMistralApiWorker directly
    const fetchPromise = callMistralApiWorker(model, messages, env); // Pass env
    ...
    if (prompt.includes('Agent 1:') && retryCount < maxRetries && !(streamCallback && prompt.includes('Agent 3:'))) {
      try {
        ...
        return callAgent(model, prompt, input, env, retryCount + 1, streamCallback); // Pass env and streamCallback on retry
      }
    }
...
export async function orchestrateAgents(userQuery, agentSelectionType, streamCallback = null, logCallback = null, env) { // Pass env
  ...
  while (true) {
    ...
    const agent1ResponseRaw = await callAgent(agent1Model, agent1SystemPrompt, agent1Input, env); // Pass env
    ...
    if (classification && (classification === 'conversational' || classification === 'math' || classification === 'code' || classification === 'unclear' || classification === 'direct')) {
      ...
      const finalResponse = await callAgent(agent3Model, agent3SystemPrompt, { rawQuery: userQuery, query: queryForAgent3, classification: classification }, env, 0, streamCallback); // Pass env
    ...
    if (action === 'search') {
      ...
      if (search_plan && search_plan.length > 0) {
        ...
        const searchPromises = search_plan.map(async (step) => {
          ...
          if (step.tool === 'serper_web_search' || step.tool === 'serper_news_search') {
            const result = await executeTool(step.tool, step.query, step.params, env, callAgent); // Pass env and callAgent
        ...
        if (wikipediaLinksInWebResults.length > 0) {
          ...
          processedWikiData = await wikiEye(wikipediaLinksInWebResults, userQuery, callAgent); // Pass callAgent
      ...
      const finalResponse = await callAgent(agent3Model, agent3SystemPrompt, { ...dataForAgent3 }, env, 0, streamCallback); // Pass env
