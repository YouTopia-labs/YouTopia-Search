...
`;
    ...
    const handleSearch = async (query, sourceElement = null) => {
        ...
        if (isBottomSearch) {
            // Bottom search box defaults to Amaya (mistral-small-latest)
            selectedModel = 'Amaya';
        } else {
        ...
        // Clear results container and sources container
        resultsContainer.innerHTML = '';
        sourcesContainer.innerHTML = ''; // Clear sources container on new search
        ...
        try {
            ...
            const streamCallback = (chunk) => {
                ...
                resultsContainer.scrollTop = resultsContainer.scrollHeight; // Scroll to bottom
            ...
            await orchestrateAgents(query, selectedModel.toLowerCase(), streamCallback, logCallback);

            // Add export panel after response completion
            ...
            setSendButtonState(false); // Reset button state
            const logContainer = document.getElementById('live-log-container');
            if (logContainer) {
                logContainer.classList.remove('is-active');
                logContainer.classList.add('is-done');
                // Remove the is-done class after 1 second to make the green line disappear
                setTimeout(() => {
                    logContainer.classList.remove('is-done');
                }, 1000);
                await new Promise(resolve => setTimeout(resolve, 1200)); // Brief delay before collapsing
            ...
            if (error.name === 'AbortError') {
                ...
                logStep(`<i class="fas fa-exclamation-triangle" style="color: #EF4444;"></i> Error: ${error.message}`);
            }

            // Always set button state back to enabled, even on error
            setSendButtonState(false);
            const logContainer = document.getElementById('live-log-container'); // Re-define logContainer here for error path
            if (logContainer) {
                logContainer.classList.remove('is-active');
                logContainer.classList.add('has-error');
                await new Promise(resolve => setTimeout(resolve, 1200)); // Keep this for collapsing
                logContainer.classList.add('collapsed');
                // New: Make the error highlight disappear after 2 seconds
                setTimeout(() => {
                    logContainer.classList.remove('has-error');
                }, 1000);

                // Ensure event listeners are correctly set up after log becomes visible
                // The mainContent click listener handles toggling the log. No need for additional setup here.
            }
            // Append error message after the log
            resultsContainer.insertAdjacentHTML('beforeend', `<div class="ai-response"><h3>⚠️ Search Error</h3><p>Sorry, something went wrong. Please try again.</p><details><summary>Details</summary><pre>${error.message}</pre></details></div>`);
        }
    };
